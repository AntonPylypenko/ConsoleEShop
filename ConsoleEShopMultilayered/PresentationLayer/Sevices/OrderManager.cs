...
namespace PresentationLayer.Sevices
{
    class OrderManager
    {
        ...
        public static OrderStatuses ChoosenStatus()
        {
            ...
            switch (choiceOfUser)
            {
                case 1:
                    ...
                    break;
                case 2:
                    return OrderStatuses.PaymentIsReceived;
                    break;
                case 3:
                    return OrderStatuses.Received;
                    break;
                case 4:
                    return OrderStatuses.Dispatched;
                    break;
                case 5:
                    return OrderStatuses.Finished;
                    break;
                case 6:
                    return OrderStatuses.CanceledByAdmin;
                    break;
                default:
        ...
        public static void MakeAllNewOrdersCanceled()
        {
            ...
            {
                if (AccountManager.CurrentUser.ID == item.Account.ID && item.Status == OrderStatuses.New)
                {
                    if (AccountManager.CurrentUser is RegisteredUser)
                    {
                        item.Status = OrderStatuses.CanceledByUser;
                    }
                    else if (AccountManager.CurrentUser is Administrator)
                    {
                        item.Status = OrderStatuses.CanceledByAdmin;
                    }
                }
            }
            Console.WriteLine("Please, type enter to continue...)");
